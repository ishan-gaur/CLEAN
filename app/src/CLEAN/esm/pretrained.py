...
def load_hub_workaround(url):
    try:
        data = torch.hub.load_state_dict_from_url(url, progress=True, map_location="cpu")
    except RuntimeError:
...
def _load_model_and_alphabet_core_v1(model_data):
    import CLEAN.esm  # since esm.inverse_folding is imported below, you actually have to re-import esm here

    alphabet = CLEAN.esm.Alphabet.from_architecture(model_data["args"].arch)
    ...
    if model_data["args"].arch == "roberta_large":
        ...
        model_type = CLEAN.esm.ProteinBertModel
        ...
        model_type = CLEAN.esm.MSATransformer
